...
Figure \ref{fig:tied notes} shows an two untied and one tied note. In MusicXML, both of these would be represented with two distinct \texttt{<note>} tags, even though the tied note is essentially equivalent to a single note.
...
When a sound is played in a room, it will reflect, and be absorbed by, surfaces in the room. These reflections cause an echo-like effect to be heard, and as the sound waves are absorbed, the reverberation becomes diminish until they become inaudible. Figure \ref{fig:reverb} shows visually what reverb looks like, where the tail of a sound is significantly extended.

\begin{figure}[h]\label{fig:reverb}
  \centering
