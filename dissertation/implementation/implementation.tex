...
|   |   test_data/
|   |   contains data used for unit tests
|   |-- irs/
...
`-- src/
    ...
    |         dicussed in section 3.5
    |-- data_synthesis/
    |   contains all of the data synthesis implementation
    |     discussed in section 3.3
    `-- test/
        contains unit tests for the classifier code
\end{verbatim}
\caption{Repository overview}
\label{fig:repositoryoverview}
\end{figure}

All of the code was written by the dissertation author. All files in the \texttt{res/data/} and \texttt{res/test\_data/} directories were created by the dissertation author, and all other files in the \texttt{res/} directory were found online.

In particular, files in the \texttt{res/irs/} directory were taken from the University of York's \href{https://www.openairlib.net}{OpenAir} project under the Creative Commons 4.0 License\footnote{\url{https://creativecommons.org/licenses/by/4.0/}}. Files in the \texttt{res/noise/}, \texttt{res/scores/}, and \texttt{res/soundfonts/} directories were taken using the Creative Commons 1.0 license.
...
MusicXML is a file format designed to store music scores for computer-readability \cite{musicxml}. It specifies a number of XML tags and their usage relating to musical structure. Figure \ref{fig:musicxml} gives some example MusicXML representing 3 equal length notes of G\musSharp{}3, C\musSharp{}4, and E4.
...
\begin{figure}[h]
  ...
  \includegraphics[scale=0.25]{spectrogram}
...
In fact, all we do is run our beat tracking algorithm on our audio and take the first-order and second-order differences between our beat times, and then apply smoothing to each by taking the moving average to reduce the impact of micro-variations in note timings (which we instead capture with the note offsets metric). Since the note offsets metric (and other metrics) require the onset function, we also memoise this computation for performance.

There were a few other choices for constructing this metric, for example instead of taking the first-order difference of our beat times, we could simply use the beat times themselves. To see the issues with this approach, suppose we have two performances that have very similar tempos in the latter half, but are different in the first half. We would like to consider these performances reasonably similar since the tempos are similar in the latter half, but by taking the absolute beat times we are unable to find any similarity since the differing tempos in the first half will offset the beat times in the latter half. Taking the first-order difference avoids this issue. Another option might be taking higher-order differences, although third-order differences and higher are generally not useful as performers are unlikely to think about such notions in their tempo.
...
\begin{enumerate}[label=\textbf{Problem \arabic*:}]
  ...
  \item
    Humans perceive different frequencies at different volumes, even for the same intensity. This is important because we want to suppress what is recorded but not heard, for example if the piano has defects that cause it to generate loud, low frequencies on some notes.
\end{enumerate}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.25]{itu468} 
...
As before, we begin by truncating the longer metric if necessary. Then, we perform a step of normalisation: we only care about relative difference between the two metrics, so we normalise both arrays such that the value of 1 represents the highest value found in either metric.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.5\textwidth}
    \centering
    \includegraphics[scale=0.21]{offsetdist1}
    \subcaption{}
  \end{minipage}%
  \begin{minipage}{0.5\textwidth}
    \centering
    \includegraphics[scale=0.21]{offsetdist2}
    \subcaption{}
  \end{minipage}
...
We can see that if $\sigma_1 = \sigma_2$ and $\mu_1 = \mu_2$, that this symmetrised divergence becomes 0, and as the parameters begin to differ, the divergence will get larger. This means that we can calculate our final similarity score as the negative exponential of our symmetrised divergence, which will be 1 when the distributions are the same, and approach 0 as the distributions differ.
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.5\textwidth}
    \centering
    \includegraphics[scale=0.27]{saxc}
    \subcaption{}
  \end{minipage}%
  \begin{minipage}{0.5\textwidth}
    \centering
    \includegraphics[scale=0.27]{pianoc}
    \subcaption{}
  \end{minipage}
...
To quantify the spectrum of a signal, we use a standard technique\cite{prakash16} of generating mel-frequency cepstrum coefficients (MFCCs), which is often used for applications like speech recognition and music genre classification. A cepstrum is obtained by first taking the log of the magnitudes in a spectrum, and then taking its inverse Fourier transform. A mel-frequency cepstrum works simply by instead using a power spectrum (like a frequency spectrum, but we square the magnitudes), with the frequency scale in mels. The MFCCs are then just the magnitudes of the inverse Fourier transformed signal.
...
Instead of creating a complex system to synthesise background noise, that will inevitably be not completely alike to real background noise, we instead just take pre-recorded samples of background noise and add these samples to our performances.
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=0.25]{reverb}
