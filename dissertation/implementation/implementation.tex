...
So, from the offsets we generate the mean and standard deviation for a normal distribution, representing when they are most likely to play a note relative to the expected beat time. Figures \ref{fig:offsetdist1} and \ref{fig:offsetdist2} show plots of inferred probability distributions for different performers and the same performer, respectively.

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.3]{offsetdist1}
  \caption{Inferred probability distributions for different performers}
  \label{fig:offsetdist1}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.3]{offsetdist2}
  \caption{Inferred probability distributions for the same performer}
  \label{fig:offsetdist2}
\end{figure}
...
% todo: change tempo metric to include step 1/2/... algorithm as in other metrics
